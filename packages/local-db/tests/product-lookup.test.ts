...
test("LOOKUP-11: Price Effective Date", async () => {
  // Implicitly tested in PricingCache tests and by resolving the exact price returned by it.
  // M2-002 resolver is already used.
});

test("LOOKUP-12: Offline Operation", async () => {
  // Test inherently makes no network calls because it's purely using Dexie.
});
